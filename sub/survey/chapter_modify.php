...
$o_table=new SurveySubject($_GET['id']);
?>
...
<input type="hidden" name="Ajax_FunName" value="ChapterModify"/>
	...
	<tbody>
		...
		<tr>
			...
			<td class="TableData"><input id="Vcl_Title" name="Vcl_Title"
				class="BigInput" style="width:300px;" size="16" maxlength="255" type="text" value="<?php echo($o_table->getTitle())?>"/>
				</td>
		</tr>
		<tr>
...
<script type="text/javascript" language="javascript">
	...
	<?php 
			$a_type=json_decode($o_table->getType());
			$s_type='';
			for($j=0;$j<count($a_type);$j++)
			{
				echo('$("#Vcl_Type"+'.$a_type[$j].').attr("checked", true);');
			}
			
	?>
	var scope=<?php echo($o_table->getScope())?>;
	$('#type_0').hide()
	$('#type_1').hide()
	$('#type_2').hide()
