...
if ($o_subject->getScope()==0)
{
	if ($_GET['bu']==1)
	{
		echo ('<script>location=\'index_start_score_yey.php?bu=1&id='.$o_total->getId().'\'</script>');
	}else{
		echo ('<script>location=\'index_start_score_yey.php?id='.$o_subject->getId().'&already='.$_GET['already'].'\'</script>');
	}
...
if ($o_subject->getScope()==2)
{
	if ($_GET['bu']==1)
	{
		echo ('<script>location=\'index_start_score_xy.php?bu=1&id='.$o_total->getId().'\'</script>');
	}else{
		echo ('<script>location=\'index_start_score_xy.php?id='.$o_subject->getId().'&already='.$_GET['already'].'\'</script>');
	}
}
