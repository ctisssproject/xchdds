...
<input type="hidden" name="Vcl_BackUrl" value="<?php echo(str_replace ( substr( $_SERVER['PHP_SELF'] , strrpos($_SERVER['PHP_SELF'] , '/')+1 ), '', $_SERVER['PHP_SELF']).'item.php?id='.$_GET['id'])?>"/>
<input type="hidden" name="Vcl_Id" value="<?php echo($_GET['id'])?>"/>
<input type="hidden" name="Ajax_FunName" value="ItemSingleAdd"/>
	...
	<tbody>
		...
		<tr>
			...
			<td class="TableData">
				...
				$o_type->PushWhere ( array ('&&', 'Type', '=', $o_object->getScope() ) );
		...
		<tr class="TableControl" align="center">
			<td colspan="2" nowrap="nowrap" height="40"><input value="提交" class="BigButtonA"
				...
				onclick="location='<?php echo(str_replace ( substr( $_SERVER['PHP_SELF'] , strrpos($_SERVER['PHP_SELF'] , '/')+1 ), '', $_SERVER['PHP_SELF']).'item.php?id='.$_GET['id'])?>'" type="button" />
				</td>
		</tr>
