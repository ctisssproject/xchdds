...
if (is_numeric ( $_GET ['articleid'])&& $_GET ['articleid']>0) {
	$n_columnid = $_GET ['articleid'];
} else {
	echo ('<script>location=\'home_footer.php\'</script>');
	exit ( 0 );
}
$o_focus = new Home_Article_Footer ( $n_columnid );
...
<form method="post" id="dialog_form"
	action="include/bn_submit.svr.php?function=ModifyFooter"
	...
	<tbody>
		<tr>
			...
			<td class="TableData"><input id="Vcl_Title" name="Vcl_Title"
				...
				value="<?php
				echo ($o_focus->getTitle())?>" type="text" style="font-size:14px; height:20px"/><input
				id="Vcl_ArticleId" name="Vcl_ArticleId" size="20" maxlength="20"
				class="BigInput" value="<?php
				echo ($o_focus->getArticleId ())?>"
				type="text" style="display: none" /></td>
		</tr>
		<tr>
			<td class="TableData" nowrap="nowrap" width="120"
				style="vertical-align: top;">文章内容：</td>
			<td class="TableData"><script id="editor" type="text/plain"><?php echo($o_focus->getContent ())?></script>
			<textarea class="BigInput" style="" name="Vcl_Content" id="Vcl_Content"
				rows="7" cols="70" style="display:none"></textarea></td>
		</tr>
		<tr>
