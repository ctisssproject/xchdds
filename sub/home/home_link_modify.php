...
if (is_numeric ( $_GET ['linkid'])&& $_GET ['linkid']>0) {
	$n_columnid = $_GET ['linkid'];
} else {
	echo ('<script>location=\'home_link.php\'</script>');
	exit ( 0 );
}
$o_focus = new Home_Link ( $n_columnid );
...
<form method="post" id="dialog_form"
	action="include/bn_submit.svr.php?function=ModifyLink"
...
<table class="TableTop" width="600" align="center">
	<tbody>
		<tr>
			<td class="left"></td>
			<td class="center subject">修改友情链接</td>
			<td class="right"></td>
		</tr>
	</tbody>
</table>
	...
	<tbody>
		<tr>
			...
			<td class="TableData"><input id="Vcl_Name" name="Vcl_Name"
				...
				value="<?php
				echo ($o_focus->getName())?>" type="text" /><input
				id="Vcl_LinkId" name="Vcl_LinkId" size="20" maxlength="20"
				class="BigInput" value="<?php
				echo ($o_focus->getLinkId ())?>"
		...
		<tr>
			...
			<td class="TableData"><input id="Vcl_Url" name="Vcl_Url"
				...
				value="<?php
				echo ($o_focus->getUrl ())?>" type="text" /></td>
		</tr>
		<tr>
