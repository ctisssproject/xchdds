...
echo ($o_list->getZcTaskList($o_page));
?>
</div>
...
<div id="master_box"
	...
	<script type="text/javascript" language="javascript">
		function search()
		{
			if($('#Vcl_Owner').val()=='' && $('#Vcl_StartDate').val()=='' && $('#Vcl_EndDate').val()=='' && $('#Vcl_SchoolName').val()=='' && $('#Vcl_Read').val()=='')
			{
				 parent.parent.Dialog_Message('请输入搜索条件 ！');
				 return;
			}
			location='gpdd_zc_task.php?owner='+$('#Vcl_Owner').val()+'&start='+$('#Vcl_StartDate').val()+'&end='+$('#Vcl_EndDate').val()+'&schoolname='+$('#Vcl_SchoolName').val()+'&read='+$('#Vcl_Read').val();
		}
	S_Root='../../';
    </script>
</body>
