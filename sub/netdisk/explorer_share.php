...
if ($o_folder->getFolderName () == FALSE) {
	...
	if ($o_folder->getShareUid () == '') {
		//没有到顶
		$s_up_button = '<td>
                	<div class="up" onclick="goUp_Share(' . $o_folder->getParentId () . ',\'folderid\')">
                	</div>
            	</td>';
	} else {
		//到顶
		$s_up_button = '<td>
                	<div class="up" onclick="goUp_Share(' . $o_folder->getUid() . ',\'uid\')">
                	</div>
            	</td>';
	}
...
<script type="text/javascript"
	src="../../js/jquery/jquery-ui.custom.min.js"></script>
...
<table border="0" cellpadding="0" cellspacing="0" class="folder_top">
	<tr>
            ...
            <td>
		<div class="copy"
			onclick="document.getElementById('filelist').contentWindow.copyAndMoveAll('copy')">
		</div>
		</td>
		<td style="width: 98%; text-align: right; font-family: 宋体"></td>
	</tr>
</table>
<table border="0" cellpadding="0" cellspacing="0" class="folder_title">
	<tr>
		<td style="width: 20px"><input id="Checkbox1" type="checkbox"
			onclick="document.getElementById('filelist').contentWindow.selectAll(this)" />
		</td>
		<td><img src="images/icon_sort.png" align="absmiddle" alt="" />&nbsp;&nbsp;文件名
		</td>
		<td style="width: 140px">大小</td>
		<td style="width: 140px">上传日期</td>
	</tr>
</table>
<table border="0" cellpadding="0" cellspacing="0" id="center"
	style="width: 100%">
	<tr>
		<td><iframe id="filelist"
			src="filelist_share.php?folderid=<?php
			echo ($n_folder);
			?>&uid=<?php
			echo ($_GET ['uid']);
			?>"
			marginwidth="0" marginheight="0" frameborder="0" framespacing="0"
			border="0" allowtransparency="true"></iframe></td>
	</tr>
</table>
