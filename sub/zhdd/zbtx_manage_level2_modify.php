...
if ($_GET['id']>0)
{
	$o_level2=new Zhdd_Zbtx_Level2($_GET['id']);
	$o_level1=new Zhdd_Zbtx_Level1($o_level2->getLevel1Id());
	$s_project_id=$o_level1->getProjectId();
	$s_level1_id=$o_level2->getLevel1Id();
}else{
	$o_level1=new Zhdd_Zbtx_Level1($_GET['level1_id']);
	$s_level1_id=$o_level1->getId();
	$s_project_id=$o_level1->getProjectId();
}
echo($s_project_id);
?>"/>
<input type="hidden" name="Vcl_Level1Id" value="<?php echo($_GET['level1_id'])?>"/>
<input type="hidden" name="Vcl_Id" value="<?php echo($_GET['id'])?>"/>
<input type="hidden" name="Vcl_FunName" value="<?php 
	if ($_GET['id']>0)
	{
		echo('ZbtxProjectLevel2Modify');
	}else{
		echo('ZbtxProjectLevel2Add');
	}
?>"/>
	...
	<tbody>
		...
		<tr>
			...
			<td class="TableData"><select name="Vcl_Number" id="Vcl_Number" class="BigSelect">
				...
				$o_table->PushWhere ( array ('&&', 'Level1Id', '=', $s_level1_id) );
		...
		<tr class="TableControl" align="center">
			<td colspan="2" nowrap="nowrap" height="40"><input value="提交" class="BigButtonA"
				onclick="zbtx_manage_level2_submit()" type="button" />&nbsp;&nbsp;&nbsp;&nbsp;
...
<script type="text/javascript" language="javascript">
	...
	{
		?>
		$('#Vcl_Name').val('<?php echo($o_level2->getName())?>');
		$('#Vcl_Number').val('<?php echo($o_level2->getNumber())?>');
		<?php
	}
	?>
    </script>
