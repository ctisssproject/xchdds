...
if ($_GET['id']>0)
{
	$o_level3=new Zhdd_Zbtx_Level3($_GET['id']);
	$o_level2=new Zhdd_Zbtx_Level2($o_level3->getLevel2Id());
	$o_level1=new Zhdd_Zbtx_Level1($o_level2->getLevel1Id());
	$s_project_id=$o_level1->getProjectId();
	$s_level2_id=$o_level2->getId();
}else{
	$o_level2=new Zhdd_Zbtx_Level2($_GET['level2_id']);
	$o_level1=new Zhdd_Zbtx_Level1($o_level2->getLevel1Id());
	$s_level2_id=$o_level2->getId();
	$s_project_id=$o_level1->getProjectId();
}
echo($s_project_id);
?>"/>
<input type="hidden" name="Vcl_Level2Id" value="<?php echo($_GET['level2_id'])?>"/>
<input type="hidden" name="Vcl_Id" value="<?php echo($_GET['id'])?>"/>
<input type="hidden" name="Vcl_FunName" value="<?php 
	if ($_GET['id']>0)
	{
		echo('ZbtxProjectLevel3Modify');
	}else{
		echo('ZbtxProjectLevel3Add');
	}
?>"/>
	...
	<tbody>
		...
		<tr>
			...
			<td class="TableData"><select name="Vcl_Number" id="Vcl_Number" class="BigSelect">
				...
				$o_table->PushWhere ( array ('&&', 'Level2Id', '=', $s_level2_id) );
		...
		<tr class="TableControl" align="center">
			<td colspan="2" nowrap="nowrap" height="40"><input value="提交" class="BigButtonA"
				onclick="zbtx_manage_level3_submit()" type="button" />&nbsp;&nbsp;&nbsp;&nbsp;
...
<script type="text/javascript" language="javascript">
	...
	{
		?>
		$('#Vcl_Name').val('<?php echo($o_level3->getName())?>');
		$('#Vcl_Number').val('<?php echo($o_level3->getNumber())?>');
		$('#Vcl_Score').val('<?php echo($o_level3->getScore())?>');
		<?php
	}
	?>
    </script>
