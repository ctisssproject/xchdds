...
$o_table=new Base_Group($_GET['id']);
?>
...
<form method="post" id="dialog_form"
	action="include/bn_submit.svr.php?function=GroupModify"
	enctype="multipart/form-data" target="ajax_submit_frame"
	style="width: 100%">
	<input type="hidden" name="Vcl_BackUrl" value="<?php echo($_SERVER['HTTP_REFERER'])?>"/>
	<input type="hidden" name="Vcl_Id" value="<?php echo($o_table->getId())?>"/>
...
<table class="TableBlock_Editor" align="center" width="1000" style="margin-top:10px">
	<tbody>
		...
		<tr class="TableControl" align="center">
			...
			<input value="提交" class="BigButtonA"
...
<script type="text/javascript" language="javascript">
	S_Root='../../';
	<?php 
		echo('document.getElementById("Vcl_Name").value="'.$o_table->getName().'";');
		$o_temp=new Base_Group_Member();
		$o_temp->PushWhere ( array ('&&', 'GroupId', '=', $o_table->getId() ) );
		for($j=0;$j<$o_temp->getAllCount();$j++)
		{
			if ($o_temp->getType($j)=='Dept')
			{
				echo('document.getElementById("Vcl_Dept_'.$o_temp->getUid($j).'").checked=true;');
			}else{
				echo('document.getElementById("Vcl_User_'.$o_temp->getUid($j).'").checked=true;');
			}
