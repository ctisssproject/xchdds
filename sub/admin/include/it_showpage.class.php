...
class ShowPage extends It_Basic {
	...
	public function getUserList($n_page) {
		...
		$o_role=new Home_Column();
		$o_role->PushWhere ( array ('&&', 'Parent', '=', 0 ) );
		$o_role->PushOrder ( array ('Number', 'A' ) );
		$n_count_column = $o_role->getAllCount ();
		$a_column=array();
		for($i = 0; $i < $n_count_column; $i ++) {
			array_push($a_column, array($o_role->getColumnId($i),$o_role->getName($i)));
		}
		for($i = 0; $i < $n_count; $i ++) {
			$s_state = '';
			...
			if ($s_rolename!='')
			{
				...
				<table style="padding:0px">
					<tr style="padding:0px">
						<td style="border-bottom:0px;color:#999999;	vertical-align:top;padding:0px;padding-left:12px">
							副角色：
						</td>
						<td style="border-bottom:0px;color:#999999;	vertical-align:top;padding:0px">
						'.$s_rolename.'
						</td>
					</tr>
				</table>
			...
			$s_record_list .= '
				             <tr class="TableLine1">
					                ...
					                <td nowrap="nowrap">
					                  &nbsp;&nbsp;主角色：' . $o_user->getRoleName ( $i ) . '<span class="setting" title="角色修改" onclick="userModifyRole(' . $o_user->getUid ( $i ) . ',\'' . $o_user->getUserName ( $i ) . '\',\'' . $o_user->getName ( $i ) . '\')"></span>'.$s_rolename.'
					                </td>
					                <td align="center"  nowrap="nowrap">
					                   ' . $s_state . '
					                </td>
					                <td align="center" nowrap="nowrap">
					                    <a href="user_show.php?uid=' . $o_user->getUid ( $i ) . '">详细</a>&nbsp;&nbsp;<a href="javascript:userModifyInfo(' . $o_user->getUid ( $i ) . ',\'' . $o_user->getUserName ( $i ) . '\',\'' . $o_user->getName ( $i ) . '\',\'' . $o_user->getSex ( $i ) . '\',\'' . $o_user->getEmail ( $i ) . '\',\'' . $o_user->getAmUsername ( $i ) . '\',\''.rawurlencode(json_encode($a_column)).'\',\''.rawurlencode($o_user->getGroupId($i)).'\')">修改信息</a>&nbsp;&nbsp;<a href="javascript:userResetPassword(' . $o_user->getUid ( $i ) . ',\'' . $o_user->getUserName ( $i ) . '\',\'' . $o_user->getName ( $i ) . '\')">重置密码</a>
					                </td>
					            </tr>
			';
		}
		...
		$s_html = '
			    ...
			    <table class="small" border="0" cellpadding="3" cellspacing="0" width="100%" style="margin-bottom:5px">
			        <tbody>
			            <tr>
                			<td class="title">
			                    &nbsp;&nbsp;&nbsp;&nbsp;共<span class="big4">&nbsp;' . $n_allcount . '</span>&nbsp;个用户&nbsp;&nbsp;&nbsp;&nbsp;<span class="big4">&nbsp;' . $o_temp->getAllCount() . '</span>&nbsp;个用户被停用
			                </td>
			                <td class="small1" align="right" valign="bottom">
			                    ' . $s_pagebutton . '
			                </td>
			    ...
			    <table class="TableList" width="100%">
			        <thead class="TableHeader">
					            <tr>
					                ...
					                <td align="center" nowrap="nowrap" width="100">
					                    姓名 <img src="../../images/arrow_down.gif" height="10" border="0" width="11" align="absmiddle">
					                </td>
	...
	public function getDeptList() {
		...
		$o_dept1->PushWhere ( array ('&&', 'DeptId', '>', 0 ) );
		...
		for($i = 0; $i < $n_count; $i ++) {
			$o_body .= '
		            <tr class="TableLine1">
		                ...
		                <td align="center">
		                  <a href="dept_modify.php?id='.$o_dept1->getDeptId ( $i ).'">编辑</a>&nbsp;&nbsp;<a href="javascript:deptDelete(' . $o_dept1->getDeptId ( $i ) . ');">删除</a>
		                </td>
		            </tr>
			';
			...
			for($j = 0; $j < $n_count2; $j ++) {
				...
				$o_body .= '
		            <tr class="TableLine1">
		                ...
		                <td align="center">
		                  <a href="dept_modify.php?id='.$o_dept2->getDeptId ( $j ).'">编辑</a>&nbsp;&nbsp;<a href="javascript:deptDelete(' . $o_dept2->getDeptId ( $j ) . ');">删除</a>
		                </td>
		            </tr>
			';
				...
				for($k = 0; $k < $n_count3; $k ++) {
		            ...
		            <tr class="TableLine1">
		                ...
		                <td align="center">
		                   <a href="dept_modify.php?id='.$o_dept3->getDeptId ( $k ).'">编辑</a>&nbsp;&nbsp;<a href="javascript:deptDelete(' . $o_dept3->getDeptId ( $k ) . ');">删除</a>
		                </td>
		            </tr>
			';
				}
