...
<table class="TableBlock_Editor" align="center" width="600" style="margin-top:10px">
	<tbody>
		...
		<tr>
			<td class="TableData" nowrap="nowrap" width="120">登录密码：</td>
			<td class="TableData">
		...
		<tr>
			...
			<td class="TableData">
			<input id="Vcl_Email" name="Vcl_Email" size="20" maxlength="50" class="BigInput" value="@xchjw.com" type="text"/> <span>注：系统默认发送邮件的邮箱，必须为易邮邮箱。</span>
			</td>
		</tr>
		<tr>
			<td class="TableData" nowrap="nowrap" width="120">AM用户名：</td>
			<td class="TableData">
			<input id="Vcl_Am_Username" name="Vcl_Am_Username" size="20" maxlength="20" class="BigInput" value="" type="text"/> <span>注：设置用户名后，AM即时消息可以收到提醒。</span>
			</td>
		</tr>
		<tr>
		...
		<tr>
		<td class="TableData" colspan="2" nowrap="nowrap" style="font-size:14px" align="center"><strong>信息发布设置</strong></td>
		</tr>
		<tr>
			<td class="TableData" nowrap="nowrap" width="120">可发布信息栏目：</td>
			<td class="TableData">
						<select name="Vcl_GroupId" id="Vcl_GroupId" style="min-width:100px" class="BigSelect"><option value="">所有栏目</option>
									<?php 
									//读取角色
									require_once RELATIVITY_PATH . 'include/db_table.class.php';
									...
									$s_select='';
									for($i = 0; $i < $n_count; $i ++) {
										$s_select.='<option value="' . $o_role->getColumnId($i) . '">' . $o_role->getName($i ). '</option>';
									}
									echo($s_select);
									?>
						</select>
