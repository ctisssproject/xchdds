...
function getList() 
{
		...
		for($i = 0; $i < $n_count; $i ++) {
			...
			if($o_term->getState($i)==1)
			{
				$a_button='<a href="javascript:;" onclick="location=\'appraise_manage_view_list.php?id='.$o_term->getId ( $i ).'\'">查看原题</a>&nbsp;&nbsp;<a href="javascript:;" onclick="location=\'appraise_manage_make_qrcode.php?id='.$o_term->getId ( $i ).'\'">单独二维码</a>&nbsp;&nbsp;<a href="javascript:;" onclick="location=\'appraise_manage_result_list.php?id='.$o_term->getId ( $i ).'\'">评价结果</a>&nbsp;&nbsp;<a href="javascript:;" onclick="location=\'appraise_manage_total_list.php?id='.$o_term->getId ( $i ).'\'">评价统计</a>';
			}
			/*
			$a_button='';			
