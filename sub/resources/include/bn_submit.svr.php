...
function UpLoadPicture() {
	...
	$o_operate->UpLoadPicture ($O_Session->getUid ());
...
function UploadLink() {
	...
	require_once 'bn_operate.class.php';
	$o_operate = new Operate ();
	$s_result = $o_operate->UploadLink ( $O_Session->getUid () );
	if ($o_operate->getResult () == false) {
		echo ('<script type="text/javascript" language="javascript">parent.goLoginPage(\'' . RELATIVITY_PATH . '\')</script>');
	} else {
		echo ('<script type="text/javascript" language="javascript">parent.parent.uploadLinkSuccessCallback('.$_POST ['Vcl_FolderId'] .')</script>');
	}
}
function FileRename() {
	global $O_Session;
...
function UploadTempFile() {
	...
	if ($o_operate->getResult () == false) {
		...
		echo ('<script type="text/javascript" language="javascript">parent.parent.uploadTempFileCallback("对不起，<br>文件有重名，请修改！")</script>');
	} else if ($s_result == 2) {
...
function MoveFile() {
	...
	if ($o_operate->getResult () == false) {
		...
		if ($s_result==1) {
			...
			echo ('<script type="text/javascript" language="javascript">parent.callbackMoveFile(' . $_POST ['Vcl_FileId'] . ',' . $_POST ['Vcl_FolderId'] . ',-1)</script>');
		}else{
			echo ('<script type="text/javascript" language="javascript">parent.callbackMoveFile(' . $_POST ['Vcl_FileId'] . ',' . $_POST ['Vcl_FolderId'] . ',0)</script>');
		}
	}
}
function MoveFolder() {
...
function CopyFile() {
	...
	if ($o_operate->getResult () == false) {
		...
		if ($s_result==1) {
			...
			echo ('<script type="text/javascript" language="javascript">parent.callbackCopyFile(' . $_POST ['Vcl_FileId'] . ',' . $_POST ['Vcl_FolderId'] . ',-1)</script>');
		}else{
			echo ('<script type="text/javascript" language="javascript">parent.callbackCopyFile(' . $_POST ['Vcl_FileId'] . ',' . $_POST ['Vcl_FolderId'] . ',0)</script>');
		}
	}
}
function ShareFile() {
