...
<?php 
	...
	for($i=0;$i<$n_count;$i++)
	{
		...
		for($j=0;$j<$n_count2;$j++)
		{
			...
			if ($o_answer->getAllCount()>0)
			{
				$s_item2.=$o_option->getNumber($j).'.&nbsp;&nbsp;'.$o_answer->getSum(0).' 人&nbsp;&nbsp;&nbsp;&nbsp;'.(floor($o_answer->getSum(0)/$n_sum*10000)/100).'%<br/>';
			}else{
...
for($i=0;$i<$n_count;$i++)
{
	if ($o_item->getTypeId($i)==4)
	{
		...
		{
			if ($o_answer->getAnswer($j)=='无')
			{
				continue;
			}
			echo('<div style="line-height:22px;margin-top:10px;margin-left:10px">答案'.$n_number.'：'.$o_answer->getAnswer($j).'</div>');
			$n_number++;
		}
		echo('</div>');
	}
