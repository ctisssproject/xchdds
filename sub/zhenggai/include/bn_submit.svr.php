...
function AddArticle() {
	...
	if ($o_operate->getResult () == false) {
		...
		echo ('<script type="text/javascript" language="javascript">parent.parent.parent.Dialog_Success("发布成功！");parent.location="../add.php"</script>');
	}
}
function SubmitComment() {
	...
	$o_operate = new Operate ();
	$o_user = $O_Session->getUserObject ();
	$result=$o_operate->SubmitComment ( $O_Session->getUid () );
	if ($o_operate->getResult () == false) {
		echo ('<script type="text/javascript" language="javascript">parent.goLoginPage(\'' . RELATIVITY_PATH . '\')</script>');
	}else{
