...
class Single_User extends Bn_Basic //单独用户,把所有根用户有关的进行统一操作
{
	...
	public function getRoleId() {
		return $this->O_Role->getRoleId ();//$SecRoleId1
	}
	public function getRoleId1() {
		return $this->O_Role->getSecRoleId1();//$SecRoleId1
	}
	public function getRoleId2() {
		return $this->O_Role->getSecRoleId2 ();//$SecRoleId1
	}
	public function getRoleId3() {
		return $this->O_Role->getSecRoleId3 ();//$SecRoleId1
	}
	public function getRoleId4() {
		return $this->O_Role->getSecRoleId4 ();//$SecRoleId1
	}
	public function getRoleId5() {
		return $this->O_Role->getSecRoleId5 ();//$SecRoleId1
	}
	...
	public function Deletion() //删除用户
{
	...
	public function AutoLogin($s_username, $s_password) {
		...
		if ($s_password == $this->O_Info->getPassword ()) {
	...
	public function AutoLoginForIndex($s_username, $s_password) {
		$n_uid = $this->FindUserName ( $s_username );
		if ($n_uid > 0) {
			$this->__construct ( $n_uid );
		} else {
		...
		if (md5 ( 'welcome ' . $s_password . ' to 教育城域网综合管理信息系统 !' ) == $this->O_Info->getPassword ()) {
			if ($this->O_Info->getState () == 0) {
				$this->S_ErrorReasion = SysText::Index ( 'STATE_ERROR' );
				return false;
			}
