...
class seccode {
  ...
  function getcode(){
     include_once 'validCode/global.func.php';
    ...
    include_once 'validCode/global.func.php';
		seccodeconvert($this->code);
    setcookie('VALIDCODE', $this->code, 0);
	...
	function image() {
		...
		if($this->animator == 1 && function_exists('imagegif')) {
			include_once 'validCode/gifmerge.class.php';
	...
	function ttffont() {
		...
		if($this->type && !empty($seccodettf)) {
			if(strtoupper($charset) != 'UTF-8') {
				include $this->includepath.'validCode/chinese.class.php';
