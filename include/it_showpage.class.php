...
class ShowPage extends It_Basic {
	...
	public function getGroupForAdd($n_uid) {
		...
		for($i = 0; $i < $n_count; $i ++) {
			...
			$o_user->PushWhere ( array ('&&', 'State', '=', 1 ) );
			$o_user->PushWhere ( array ('||', 'SecRoleId1', '=', $o_role->getRoleId ( $i ) ) );
			$o_user->PushWhere ( array ('&&', 'State', '=', 1 ) );
			$o_user->PushWhere ( array ('||', 'SecRoleId2', '=', $o_role->getRoleId ( $i ) ) );
			$o_user->PushWhere ( array ('&&', 'State', '=', 1 ) );
			$o_user->PushWhere ( array ('||', 'SecRoleId3', '=', $o_role->getRoleId ( $i ) ) );
			$o_user->PushWhere ( array ('&&', 'State', '=', 1 ) );
			$o_user->PushWhere ( array ('||', 'SecRoleId4', '=', $o_role->getRoleId ( $i ) ) );
			$o_user->PushWhere ( array ('&&', 'State', '=', 1 ) );
			$o_user->PushWhere ( array ('||', 'SecRoleId5', '=', $o_role->getRoleId ( $i ) ) );
			$o_user->PushWhere ( array ('&&', 'State', '=', 1 ) );
			$n_user_count = $o_user->getAllCount ();
			for($l = 0; $l < $n_user_count; $l ++) {
	...
	public function setGPDDiconNumber()
	{
		$n_sum=0;
		...
		$o_temp->PushWhere ( array ('&&', 'OwnerId', '=', $this->O_SingleUser->getUid ()) );
		...
		$o_temp->PushWhere ( array ('&&', 'OpenId', '=', $this->O_SingleUser->getUid ()) );
		...
		$o_temp=new GPDD_Wenti();
		$o_temp->PushWhere ( array ('&&', 'State', '=', 2) );		
		$a_dept=$this->O_SingleUser->getDeptId();//获取部门编号
		$o_temp->PushWhere ( array ('&&', 'SchoolId', '=',$a_dept[0]) );
		$n_sum=$n_sum+$o_temp->getAllCount();
		//3.4 等待业务科室处理
		$o_temp=new GPDD_Wenti();
		$o_temp->PushWhere ( array ('&&', 'State', '=', 3) );	
		if ($this->O_SingleUser->ValidModule(30016)==true)
		{
			$n_sum=$n_sum+$o_temp->getAllCount();
		}		
		...
		$o_temp=new GPDD_Zc();
		$o_temp->PushWhere ( array ('&&', 'State', '=', 2) );	
		if ($this->O_SingleUser->ValidModule(30024)==true)
		{
			$n_sum=$n_sum+$o_temp->getAllCount();
		}	
		//5. 专项督导-督查
		...
		$o_temp=new GPDD_Dc();
		$o_temp->PushWhere ( array ('&&', 'State', '=', 2) );
		$o_temp->PushWhere ( array ('&&', 'OwnerId', 'Like', '%"'.$this->O_SingleUser->getUid().'"%' ) );
		...
		$o_temp=new GPDD_Dc();
		$o_temp->PushWhere ( array ('&&', 'State', '=', 4) );
		$o_temp->PushWhere ( array ('&&', 'Uid', '=',$this->O_SingleUser->getUid()) );
		$n_sum=$n_sum+$o_temp->getAllCount();
		
		//开始保存计数
		$o_temp=new GPDD_Setup();
		$o_temp->PushWhere ( array ('&&', 'Uid', '=', $this->O_SingleUser->getUid ()) );
		if ($o_temp->getAllCount()>0)
		{
			$o_temp=new GPDD_Setup($this->O_SingleUser->getUid ());
		}else{
			$o_temp=new GPDD_Setup();
			$o_temp->setUid($this->O_SingleUser->getUid ());
		}		
		$o_temp->setWaitRead($n_sum);
		$o_temp->Save();
	}
	public function setHomeNumber()
	{
		require_once RELATIVITY_PATH . 'sub/home/include/db_table.class.php';
		$n_sum=0;
		if ($this->O_SingleUser->ValidModule ( 96 )) {
			require_once RELATIVITY_PATH . 'sub/home/include/db_table.class.php';
		...
		//开始保存计数
		$o_temp=new Home_Setup();
		$o_temp->PushWhere ( array ('&&', 'Uid', '=', $this->O_SingleUser->getUid ()) );
		if ($o_temp->getAllCount()>0)
		{
			$o_temp=new Home_Setup($this->O_SingleUser->getUid ());
		}else{
			$o_temp=new Home_Setup();
			$o_temp->setUid($this->O_SingleUser->getUid ());
		}		
	...
	public function getDesktop() {
		...
		for($i = 0; $i < $n_count; $i ++) {
			...
			if ($o_userModule->getPath ( $i ) != '') {
				eval ( 'require_once RELATIVITY_PATH . \'' . $o_userModule->getPath ( $i ) . 'include/db_table.class.php\';' );
				eval ( '$o_wait=new ' . $o_userModule->getWaitReadTable ( $i ) . '($this->O_SingleUser->getUid ());' );
				if ($o_wait->getWaitRead () > 9) {
					$s_classname = 'count count10';
				} else {
					$s_classname = 'count count' . $o_wait->getWaitRead ();
				}
				if ($o_wait->getWaitRead () > 0) {
					$s_function = 'addTabForRefresh';
				} else {
					$s_function = 'addTab';
				}
			} else {
		...
		$s_html = '
					<div style="top: 112px; display: none;" id="start_menu_panel">
			            ...
			            <div class="panel-user">
			                ...
			                <div class="tools">
			                    <a class="logout" href="javascript:;" onclick="menuShowAndHide();Dialog_Confirm(\'确定要注销登录吗？\',function (){location=\'index.php?loginout=true\'});" hidefocus="hidefocus" title="注销">
			                    </a>
			                </div>
			            </div>
